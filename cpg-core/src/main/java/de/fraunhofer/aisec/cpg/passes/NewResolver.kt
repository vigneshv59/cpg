...
class NewResolver : Pass() {
    ...
    override fun accept(t: TranslationResult) {
        lang?.let {
            // Update scope manager, since it can change after parallel frontends are done
            sm = it.scopeManager
            inference = Inference(it)
        }
    ...
    fun resolve(ref: DeclaredReferenceExpression, tu: TranslationUnitDeclaration): Declaration? {
        ...
        decider?.let {
            return decider.decide(symbols, ref, inference)
        }
    ...
    private fun resolveDeclaredReferenceExpression(
        ...
        return if (lang?.isQualified(name) == true) {
    ...
    private fun resolveMemberExpression(
        ...
        if (record == null) {
            record = inference?.inferRecordDeclaration(baseType, ref, tu)
        }
        ...
        val qualifiedName = lang?.qualifyName(scope, ref.name)
        ...
        for (superRecord in record.superTypeDeclarations) {
            sm.lookupScope(superRecord)?.let {
                lang?.qualifyName(it, ref.name)?.let { supers += it }
            }
        }

        // Lookup the name using the qualified name as well as its super class names
        return qualifiedName?.let {
            sm.lookupQualified(listOf(qualifiedName, *supers.toTypedArray()))
        }
            ?: listOf()
    }
...
object CallResolution : ResolutionDecision<CallExpression, DeclaredReferenceExpression> {
    ...
    override fun decide(
        ...
        if (candidates.isEmpty()) {
            val func = inference?.inferFunctionDeclaration(benefactor, source)
            if (func != null) {
